...
class Page {
	...
	var $file_root = "/var/www/nova-open/";
	...
	function getClassRoot(){
		return $this->file_root."classes/";
	}

	function getTemplateRoot(){
		return $this->file_root."templates/";
	}

	function getAcumenRoot(){
		return $this->file_root."acumen/";
	}

	function getImageRoot(){
		return $this->file_root."images/";
	}
	...
	function createTabs($url_prefix, $view, $views){
		if(!is_array($views)){die("Error!  Page::createTabs expected an array!");}
		...
		echo '<table align="center" style="border-width: 3px 0px;" cellpadding="5px" cellspacing="0px" width="80%"><tr>';
		...
		foreach($views as $v){
		    ...
		    ?><a href='<?php echo $url_prefix.$v;?>'?> <?php echo preg_replace("/_/", " ", $v);?></a></td><?php
		}
		...
		?>
		<tr><td class="current_page" colspan=<?php echo count($views)+1; ?>><?php
	}
	...
	function displayFooter($noheader=false) {
		...
		include($this->getTemplateRoot()."default_footer.html");
	...
	function printTextbox($v, $attr, $disp_type = "form") {
		...
		if($disp_type == "form"){
			?><input type=text size=<?php echo $attr['box_size']?> maxlength=255 name="<?php echo $v?>" id="<?php echo $v?>" value="<?php echo htmlspecialchars($lvar)?>"><?php
		} else {
			echo $lvar;
		}
	}
		...
		if(empty($lvar)){$lvar=$attr['default_val'];}
                ...
                if($disp_type == "form"){
                        ?><input type=password size=<?php echo $attr['box_size']?> maxlength=255 name="<?php echo $v?>" id="<?php echo $v?>" value=""><?php
                } else {
                        echo $lvar;
                }
        }
